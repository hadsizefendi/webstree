...
<template>
    <div>
        <div class="relative h-screen top-0 right-0 m-0 p-0"></div>
        <div id="home" class="overflow-hidden p-0 m-0 top-0 absolute w-full">
            <UCarousel ref="carouselRef" v-slot="{ item, index }" :items="sliderItems" :ui="{ item: 'basis-full' }"
                indicators class="pointer-events-none">
                <div class="relative w-screen h-screen top-0">
                    ...
                    <div class="absolute inset-0">
                        ...
                        <img v-else :src="item.url" class="w-full h-full object-cover" :alt="item.title" />
                    </div>
                    ...
                    <div class="relative container mx-auto h-full flex items-center justify-center">
                        <div class="max-w-3xl text-gray dark:text-white p-8">
                            <UBadge v-if="item.subtitle" variant="subtle" size="lg" class="mb-4" :label="item.subtitle"
                                v-motion="getMotion(0)" :ui="{
                            ...
                            <h2 class="text-4xl md:text-6xl font-bold mb-4" v-motion="getMotion(1)">
                                {{ item.title }}
                            </h2>

                            <p class="text-lg text-gray/80 dark:text-white/80" v-motion="getMotion(2)">
                                {{ item.description }}
                            </p>
                        </div>
                    </div>
