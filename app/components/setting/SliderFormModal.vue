        ...
        <div class="p-6 space-y-8">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-8">
                ...
                <div class="grid grid-cols-1 gap-3">
                    <UFormGroup label="Slider Title" required>
                        <UInput v-model="localState.title" @update:model-value="updateState({ title: $event })"
                            placeholder="Enter main title..." :ui="{
                                rounded: $settings.uiConfig.rounded,
                                shadow: $settings.uiConfig.shadow,
                                background: $settings.uiConfig.background,
                                ring: $settings.uiConfig.border
                    ...
                    <UFormGroup label="Description">
                        <UTextarea v-model="localState.description"
                            @update:model-value="updateState({ description: $event })"
                            placeholder="Enter general description..." :ui="{
                                rounded: $settings.uiConfig.rounded,
                                shadow: $settings.uiConfig.shadow,
                                background: $settings.uiConfig.background,
                                ring: $settings.uiConfig.border
                ...
                <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-6">
                    ...
                    <div class="flex items-center justify-center mb-4">
                        <div class="border-2 border-dotted border-gray-300 dark:border-gray-700 rounded-lg p-6 cursor-pointer transition-all duration-300 hover:border-primary-500"
                            @click="showMediaSelector = true">
                            <div class="text-center">
                                <UIcon name="i-heroicons-plus-circle" class="w-12 h-12 mx-auto text-gray-400 mb-2" />
                                <div>
                                    <h3 class="text-lg font-semibold">Add Slider Item</h3>
                                    <p class="text-sm text-gray-500">You can add and edit your images and videos</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <!-- Selected Items List -->
                        <div ref="sortableEl" class="space-y-2">
                            <div v-for="(item, index) in localState.items" :key="item.url + index"
                                ...
                                <div class="flex items-start gap-1 drag-handle cursor-grab group relative">
                                    ...
                                    <div class="flex-1 flex gap-2 items-center justify-center z-10">
                                        ...
                                        <div class="flex-grow space-y-2 pb-2">
                                            <UInput variant="none" v-model="item.title" placeholder="Add title..."
                                                class="bg-black/60 m-2 backdrop-blur-sm p-2" :ui="{
                                                    rounded: $settings.uiConfig.rounded,
                                                    shadow: $settings.uiConfig.shadow,
                                                    background: $settings.uiConfig.background,
                                                    ring: $settings.uiConfig.border
                                                }" />
                                            <UInput variant="none" v-model="item.subtitle" placeholder="Add subtitle..."
                                                class="bg-black/60 m-2 backdrop-blur-sm p-2" :ui="{
                                                    rounded: $settings.uiConfig.rounded,
                                                    shadow: $settings.uiConfig.shadow,
                                                    background: $settings.uiConfig.background,
                                            ...
                                            <UTextarea variant="none" v-model="item.description"
                                                placeholder="Add description..."
                                                class="bg-black/60 m-2 backdrop-blur-sm p-2" :rows="3" :ui="{
                                                    rounded: $settings.uiConfig.rounded,
                                                    shadow: $settings.uiConfig.shadow,
                                                    background: $settings.uiConfig.background,
                                                    ring: $settings.uiConfig.border
                                ...
                                <UBadge :label="(index + 1).toString()" color="cyan" size="xs" variant="solid"
                                    class="w-6 h-6 flex items-center justify-center absolute -top-1 -right-1 z-30" :ui="{
                                        rounded: $settings.uiConfig.rounded,
                                        shadow: $settings.uiConfig.shadow,
                                        background: $settings.uiConfig.background,
                                        ring: $settings.uiConfig.border
                                ...
                                <UButton color="red" variant="solid" icon="i-heroicons-trash" size="xs"
                                    ...
                                    class="w-6 h-6 flex items-center justify-center absolute -top-1 -left-1 z-30" :ui="{
                                        rounded: $settings.uiConfig.rounded,
                                        shadow: $settings.uiConfig.shadow,
                                        background: $settings.uiConfig.background,
                                        ring: $settings.uiConfig.border
            ...
            <UModal v-if="showMediaSelector" v-model="showMediaSelector" :persistent="true" :ui="{
                rounded: $settings.uiConfig.rounded,
                shadow: $settings.uiConfig.shadow,
                background: $settings.uiConfig.background,
                ring: $settings.uiConfig.border
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <UButton color="gray" variant="ghost" icon="i-heroicons-x-mark"
                            @click="showMediaSelector = false" :ui="{
                                rounded: $settings.uiConfig.rounded,
                                shadow: $settings.uiConfig.shadow,
                                background: $settings.uiConfig.background,
                                ring: $settings.uiConfig.border
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-4">
                        <UButton color="gray" block variant="soft" @click="showMediaSelector = false" :ui="{
                            rounded: $settings.uiConfig.rounded,
                            shadow: $settings.uiConfig.shadow,
                            background: $settings.uiConfig.background,
                            ring: $settings.uiConfig.border
                        ...
                        <UButton color="primary" block :loading="adding" :disabled="selectedMedias.length === 0"
                            @click="handleMediaSelect(selectedMedias)" :ui="{
                                rounded: $settings.uiConfig.rounded,
                                shadow: $settings.uiConfig.shadow,
                                background: $settings.uiConfig.background,
                                ring: $settings.uiConfig.border
