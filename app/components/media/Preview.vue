  ...
  <UModal :model-value="modelValue" :ui="{
    ...
    <UCard :ui="{
      ...
      <template #header>
        <div class="flex items-center justify-between">
          <div class="flex gap-2 w-full">
            <UButton block color="yellow" label="Close" variant="soft" icon="i-heroicons-x-mark"
      ...
      <div class="relative rounded-lg bg-gray-900">
        <img v-if="media?.type === 'image'" :src="media?.url" :alt="media?.title" class="w-full h-full object-contain">
        <video v-else :src="media?.url" controls class="w-full h-full object-contain" />
      </div>
      ...
      <template #footer>
        <UButton block color="red" label="Delete" variant="soft" icon="i-heroicons-trash" @click="$emit('delete')" :ui="{
          rounded: $settings.uiConfig.rounded,
          shadow: $settings.uiConfig.shadow,
          background: $settings.uiConfig.background,
          ring: $settings.uiConfig.border
