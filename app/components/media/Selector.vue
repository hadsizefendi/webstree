<template>
  <div>
    <div class="space-y-4">
      ...
      <div class="relative group cursor-pointer w-full aspect-video" @click="showMediaModal = true">
        <!-- Video Preview -->
        <video v-if="type === 'video' && modelValue" :key="modelValue" class="w-full h-full object-cover rounded-lg"
          autoplay muted loop playsinline>
          <source :src="modelValue" type="video/mp4">
        </video>
        <!-- Image Preview -->
        <img v-else :src="modelValue || defaultImage"
        ...
        <div
          ...
          <UIcon :name="type === 'video' ? 'i-heroicons-video-camera' : 'i-heroicons-photo'"
      ...
      <UModal v-model="showMediaModal" :ui="{
        width: 'max-w-5xl',
        ...
        <div class="p-6">
          <div class="flex justify-between items-center mb-6">
            ...
            <UButton color="gray" variant="ghost" icon="i-heroicons-x-mark" @click="showMediaModal = false" :ui="{
              rounded: $settings.uiConfig.rounded,
              shadow: $settings.uiConfig.shadow,
              background: $settings.uiConfig.background,
              ring: $settings.uiConfig.border
          ...
          <div class="mb-6">
            <div
              ...
              <div class="text-center">
                <UIcon name="i-heroicons-cloud-arrow-up" class="w-8 h-8 mx-auto text-gray-400" />
                <p class="mt-2 text-sm text-gray-500">Click or drag to upload new {{ type === 'video' ? 'video' :
                  'image' }}</p>
              </div>
              <input ref="fileInput" type="file" multiple :accept="type === 'video' ? 'video/*' : 'image/*'"
          ...
          <div v-if="loading" class="grid grid-cols-4 gap-4">
            <div v-for="n in 8" :key="n" class="aspect-video animate-pulse bg-gray-200 dark:bg-gray-700 rounded-lg">
            </div>
          </div>

          <div v-else class="flex flex-wrap items-center justify-center gap-2 max-h-[400px] overflow-y-auto p-2">
            <div v-for="item in paginatedItems" :key="item._id"
              ...
              :class="{ 'ring-2 ring-primary-500': selectedUrl === item.url }" @click="selectMedia(item)">

              <!-- Video Preview -->
              <video v-if="type === 'video'" :key="item.url" class="w-32 h-32 object-cover aspect-video" autoplay muted
                loop playsinline>
                <source :src="item.url" type="video/mp4">
              </video>
              <!-- Image Preview -->
              <img v-else :src="item.url" :alt="item.title" class="w-32 h-32 object-cover aspect-video">
              ...
              <div
                ...
                <UIcon name="i-heroicons-check-circle" v-if="selectedUrl === item.url"
                  class="w-8 h-8 text-primary-500" />
                <UIcon name="i-heroicons-plus-circle" v-else class="w-8 h-8 text-white" />
              </div>
            </div>
          </div>
          ...
          <div class="mt-4 flex justify-center gap-2">
            <UButton :disabled="currentPage === 1" @click="currentPage--" color="white" variant="soft"
              icon="i-heroicons-chevron-left" :ui="{
                rounded: $settings.uiConfig.rounded,
                shadow: $settings.uiConfig.shadow,
                background: $settings.uiConfig.background,
                ring: $settings.uiConfig.border
            ...
            <UButton :disabled="currentPage === totalPages" @click="currentPage++" color="white" variant="soft"
              icon="i-heroicons-chevron-right" :ui="{
                rounded: $settings.uiConfig.rounded,
                shadow: $settings.uiConfig.shadow,
                background: $settings.uiConfig.background,
                ring: $settings.uiConfig.border
          ...
          <div class="mt-6 flex justify-end gap-3">
            <UButton color="gray" variant="soft" @click="showMediaModal = false" :ui="{
              rounded: $settings.uiConfig.rounded,
              shadow: $settings.uiConfig.shadow,
              background: $settings.uiConfig.background,
              ring: $settings.uiConfig.border
            }">Cancel</UButton>
            <UButton color="primary" @click="confirmSelection" :ui="{
              rounded: $settings.uiConfig.rounded,
              shadow: $settings.uiConfig.shadow,
              background: $settings.uiConfig.background,
              ring: $settings.uiConfig.border
