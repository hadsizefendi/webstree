  ...
  <div class="min-h-screen">
    <UContainer class="py-12">
      ...
      <div
        ...
        <div class="flex items-center gap-2">
          <UButton color="gray" variant="soft" icon="i-heroicons-arrow-path" :loading="loading"
            class="shadow-sm hover:shadow-md transition-all" @click="refreshBlogs" :ui="{
              rounded: $settings.uiConfig.rounded,
              shadow: $settings.uiConfig.shadow,
              background: $settings.uiConfig.background,
              ring: $settings.uiConfig.border
          ...
          <UButton color="primary" variant="solid" icon="i-heroicons-plus" @click="showAddBlogForm" :ui="{
            rounded: $settings.uiConfig.rounded,
            shadow: $settings.uiConfig.shadow,
            background: $settings.uiConfig.background,
            ring: $settings.uiConfig.border
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-4">
                ...
                <MediaSelector v-model="state.imageUrl" default-image="/images/blog.webp" />
              </div>
              <div class="space-y-4">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">
                  Video URL
                </label>
                <MediaSelector v-model="state.videoUrl" type="video" />
              </div>
            </div>

            <!-- Form Fields -->
            <UFormGroup label="Blog Title" required>
              <UInput v-model="state.title" placeholder="Enter blog title..." icon="i-heroicons-document-text" :ui="{
                rounded: $settings.uiConfig.rounded,
                shadow: $settings.uiConfig.shadow,
                background: $settings.uiConfig.background,
                ring: $settings.uiConfig.border
            ...
            <UFormGroup label="Blog Content" required>
              <UTextarea v-model="state.description" placeholder="Enter blog content..." :rows="6" :ui="{
                rounded: $settings.uiConfig.rounded,
                shadow: $settings.uiConfig.shadow,
                background: $settings.uiConfig.background,
                ring: $settings.uiConfig.border
      ...
      <div class="space-y-8">
        ...
        <!-- Blogs List -->
        <div v-else-if="blogs.length > 0" class="flex flex-wrap items-center justify-center gap-4">
          <UCard v-for="blog in blogs" :key="blog._id"
            ...
            :class="{ 'ring-2 ring-primary-500': selectedItems.includes(blog._id) }" :ui="{
              body: { padding: '!p-0' },
              rounded: $settings.uiConfig.rounded,
              shadow: $settings.uiConfig.shadow,
              background: $settings.uiConfig.background,
            ...
            <div class="relative aspect-video">
              ...
              <!-- Video Indicator -->
              <div v-if="blog.videoUrl" class="absolute top-2 right-2">
                <UIcon name="i-heroicons-video-camera" class="w-6 h-6 text-white" />
              </div>
              ...
              <!-- Selection Indicator -->
              <div v-if="selectedItems.includes(blog._id)"
              ...
              <div
                ...
                <UButton color="red" variant="ghost" icon="i-heroicons-trash" size="xs"
                  @click.stop="deleteBlog(blog._id)" :ui="{
                    rounded: $settings.uiConfig.rounded,
                    shadow: $settings.uiConfig.shadow,
                    background: $settings.uiConfig.background,
                    ring: $settings.uiConfig.border
                ...
                <UButton color="purple" variant="ghost" icon="i-heroicons-check-circle" size="xs"
                  @click="toggleSelect(blog._id, $event)" :ui="{
                    rounded: $settings.uiConfig.rounded,
                    shadow: $settings.uiConfig.shadow,
                    background: $settings.uiConfig.background,
                    ring: $settings.uiConfig.border
              ...
              <div class="absolute inset-0 p-4">
                <h3 class="font-semibold text-lg text-white line-clamp-1 mb-2">{{ blog.title }}</h3>
        ...
        <div v-else class="text-center py-12">
          <UIcon name="i-heroicons-document-text" class="w-16 h-16 mx-auto text-gray-400" />
          <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-gray-100">No blog posts yet</h3>
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Get started by adding a new blog post.</p>
        </div>
