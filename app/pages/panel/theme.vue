      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div class="flex items-center gap-3">
          <UButton icon="i-heroicons-arrow-left" color="white" variant="ghost" size="sm"
            :ui="{ rounded: settings.uiConfig.rounded }" class="!rounded-full hover:bg-gray-100 dark:hover:bg-gray-800"
      ...
      <div class="flex flex-wrap justify-center gap-6">
        ...
        <UCard :ui="{
          ...
          <div class="space-y-6">
            ...
            <div class="space-y-2">
              ...
              <SettingColorPicker v-model="settings.primary" :default-tone="500" @change="updateConfig" />
            </div>

            <!-- Gray Color -->
            <div class="space-y-2">
              <UDivider>Secondary</UDivider>
              <SettingColorPicker v-model="settings.gray" :default-tone="500" @change="updateConfig" />
            </div>
          </div>
        </UCard>
        ...
        <UCard :ui="{
          ...
          <template #header>
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-lg font-semibold">UI Configuration</h3>
            </div>
          </template>
          ...
          <div class="space-y-4 pb-6">
              ...
              <div class="space-y-4">
                ...
                <div class="space-y-2">
                  <div class="flex flex-wrap gap-2">
                    <UButton color="primary" :ui="{ rounded: settings.uiConfig.rounded }">Primary</UButton>
                    <UButton color="gray" variant="solid" :ui="{ rounded: settings.uiConfig.rounded }">Gray
                    </UButton>
                    <UButton variant="soft" color="primary" :ui="{ rounded: settings.uiConfig.rounded }">Soft
                    </UButton>
                    <UButton variant="outline" color="primary" :ui="{ rounded: settings.uiConfig.rounded }">Outline
                    </UButton>
                    <UButton variant="ghost" color="primary" :ui="{ rounded: settings.uiConfig.rounded }">Ghost
                    </UButton>
                  </div>
                </div>
                ...
                <div class="space-y-2">
                  <div class="grid gap-4">
                    <UInput placeholder="Default input example" :ui="{ rounded: settings.uiConfig.rounded }" />
                    <UInput placeholder="Input with icon" icon="i-heroicons-magnifying-glass"
                      :ui="{ rounded: settings.uiConfig.rounded }" />
                    <UTextarea placeholder="Textarea example" :ui="{ rounded: settings.uiConfig.rounded }" />
                    <USelect :options="['Option 1', 'Option 2', 'Option 3']" placeholder="Select example"
                      :ui="{ rounded: settings.uiConfig.rounded }" />
                  </div>
                </div>
              </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-2">
                ...
                <SettingRoundedPicker v-model="settings.uiConfig.rounded" @change="updateConfig" />
              </div>
              <div class="space-y-2">
                <label class="block text-sm font-medium text-left">Shadow</label>
                <SettingShadowPicker v-model="settings.uiConfig.shadow" @change="updateConfig" />
              </div>
              <div class="space-y-2">
                <label class="block text-sm font-medium text-left">Background</label>
                <SettingBackgroundPicker v-model="settings.uiConfig.background" @change="updateConfig" />
              </div>
              <div class="space-y-2">
                <label class="block text-sm font-medium text-left">Border</label>
                <SettingBorderPicker v-model="settings.uiConfig.border" @change="updateConfig" />
              </div>
            </div>
          </div>
        </UCard>
        <UCard :ui="{
          ...
          <template #header>
            <div class="flex items-center gap-2">
              <UIcon name="i-heroicons-cog-6-tooth" class="w-5 h-5" />
              <h3 class="text-lg font-semibold">Page Transition</h3>
            </div>
          </template>
          <label class="block text-sm font-medium">Transition</label>
          <SettingTransitionPicker v-model="settings.uiConfig.transition" @change="updateConfig" />
        </UCard>
      </div>
    </UContainer>
  </div>
